# API Reference

Auto-generated API documentation for key Hytale server classes.

::: tip
This is auto-generated from the source code. For usage examples, see the [Reference guides](/reference/events/damage).
:::

## Classes

- [ItemStack](/api/ItemStack) - `com.hypixel.hytale.server.core.inventory`
- [Inventory](/api/Inventory) - `com.hypixel.hytale.server.core.inventory`
- [ItemContainer](/api/ItemContainer) - `com.hypixel.hytale.server.core.inventory.container`
- [LivingEntity](/api/LivingEntity) - `com.hypixel.hytale.server.core.entity`
- [Player](/api/Player) - `com.hypixel.hytale.server.core.entity.entities`
- [TransformComponent](/api/TransformComponent) - `com.hypixel.hytale.server.core.modules.entity.component`
- [Damage](/api/Damage) - `com.hypixel.hytale.server.core.modules.entity.damage`
- [DamageModule](/api/DamageModule) - `com.hypixel.hytale.server.core.modules.entity.damage`
- [DamageSystems](/api/DamageSystems) - `com.hypixel.hytale.server.core.modules.entity.damage`
- [DeathComponent](/api/DeathComponent) - `com.hypixel.hytale.server.core.modules.entity.damage`
- [DeathSystems](/api/DeathSystems) - `com.hypixel.hytale.server.core.modules.entity.damage`
- [RespawnSystems](/api/RespawnSystems) - `com.hypixel.hytale.server.core.modules.entity.damage`
- [DamageCause](/api/DamageCause) - `com.hypixel.hytale.server.core.modules.entity.damage`
- [EntityStatMap](/api/EntityStatMap) - `com.hypixel.hytale.server.core.modules.entitystats`
- [JavaPlugin](/api/JavaPlugin) - `com.hypixel.hytale.server.core.plugin`
- [PlayerRef](/api/PlayerRef) - `com.hypixel.hytale.server.core.universe`
- [Message](/api/Message) - `com.hypixel.hytale.server.core`
- [RefSystem](/api/RefSystem) - `com.hypixel.hytale.component.system`
- [RefChangeSystem](/api/RefChangeSystem) - `com.hypixel.hytale.component.system`
- [EntityEventSystem](/api/EntityEventSystem) - `com.hypixel.hytale.component.system`
- [EntityTickingSystem](/api/EntityTickingSystem) - `com.hypixel.hytale.component.system.tick`
- [Query](/api/Query) - `com.hypixel.hytale.component.query`
- [CommandBuffer](/api/CommandBuffer) - `com.hypixel.hytale.component`
- [Store](/api/Store) - `com.hypixel.hytale.component`
- [Ref](/api/Ref) - `com.hypixel.hytale.component`
- [NPCEntity](/api/NPCEntity) - `com.hypixel.hytale.server.npc.entities`
- [NPCPlugin](/api/NPCPlugin) - `com.hypixel.hytale.server.npc`
- [WorldConfig](/api/WorldConfig) - `com.hypixel.hytale.server.core.universe.world`
- [PlayerChatEvent](/api/PlayerChatEvent) - `com.hypixel.hytale.server.core.event.events.player`
- [CustomUIPage](/api/CustomUIPage) - `com.hypixel.hytale.server.core.entity.entities.player.pages`
- [BasicCustomUIPage](/api/BasicCustomUIPage) - `com.hypixel.hytale.server.core.entity.entities.player.pages`
- [InteractiveCustomUIPage](/api/InteractiveCustomUIPage) - `com.hypixel.hytale.server.core.entity.entities.player.pages`
- [CustomUIHud](/api/CustomUIHud) - `com.hypixel.hytale.server.core.entity.entities.player.hud`
- [EntityUIComponent](/api/EntityUIComponent) - `com.hypixel.hytale.server.core.modules.entityui.asset`
- [EntityStatUIComponent](/api/EntityStatUIComponent) - `com.hypixel.hytale.server.core.modules.entityui.asset`
- [CombatTextUIComponent](/api/CombatTextUIComponent) - `com.hypixel.hytale.server.core.modules.entityui.asset`
- [UICommandBuilder](/api/UICommandBuilder) - `com.hypixel.hytale.server.core.ui.builder`
- [UIEventBuilder](/api/UIEventBuilder) - `com.hypixel.hytale.server.core.ui.builder`
- [Anchor](/api/Anchor) - `com.hypixel.hytale.server.core.ui`
- [Area](/api/Area) - `com.hypixel.hytale.server.core.ui`
- [DropdownEntryInfo](/api/DropdownEntryInfo) - `com.hypixel.hytale.server.core.ui`
- [ItemGridSlot](/api/ItemGridSlot) - `com.hypixel.hytale.server.core.ui`
- [LocalizableString](/api/LocalizableString) - `com.hypixel.hytale.server.core.ui`
- [PatchStyle](/api/PatchStyle) - `com.hypixel.hytale.server.core.ui`
- [Value](/api/Value) - `com.hypixel.hytale.server.core.ui`
- [ServerFileBrowser](/api/ServerFileBrowser) - `com.hypixel.hytale.server.core.ui.browser`
- [HytaleServer](/api/HytaleServer) - `com.hypixel.hytale.server.core`
- [GenericBlockBulkUpdater](/api/GenericBlockBulkUpdater) - `com.hypixel.hytale.server.core.universe.world`
